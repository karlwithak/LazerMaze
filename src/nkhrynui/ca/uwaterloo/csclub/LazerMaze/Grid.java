...
public class Grid {
    ...
    void makeGrid() {
    	...
    	for (int i = 0; i < (5  +(2 * Math.sqrt(level.num))) * linesAdjust && level.num !=0; i++) {
    		...
    		if ((int)a/10 % 2 == 1) {
    			length = (int) (Math.ceil(randomBetween(-SCREENHEIGHT/LINESPACING/2,SCREENHEIGHT/LINESPACING/2)*lengthAdjust)*LINESPACING);
    			lines.add(new Line(a,b,a+length,b)); 
    		}
    		else {
    			length = (int) (Math.ceil(randomBetween(-(SCREENHEIGHT-NAVHEIGHT)/LINESPACING/2,(SCREENHEIGHT-NAVHEIGHT)/LINESPACING/2)*lengthAdjust)*LINESPACING);
